...
Do you know the Five Flags? There are six flags in LWC actually (although one is Admin only so we won't discuss it.) These flags are used to customize the functionality of your protected items. 
These flags can be set with /c[flag] on or /c[flag] off, depending on whether you want it on or off.

/chopper: This flag sets whether or not a hopper can interact with your protection, turn it on with /chopper on or off with /chopper off.
...
- Do you need to set a password lock on 1000 chests? Do you need to turn all of your doors into magnets? Want to save some time? Try /lwc mode persist on and type in a command. 
You can then go ahead and punch all of the things you want to affect with that command without having to type it in again! Just make sure you use /lwc mode persist off again afterward.
